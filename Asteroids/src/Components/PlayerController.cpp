...
void PlayerController::FixedUpdate(const sf::Time &elapsed)
{
    // Handle Collisions
    /*
        Handling Bullet hitting an Asteroid is in the Asteroid itself since it is the one that will be deactivated
        Handling the collision between the Player and the Asteroid happens here
        @TODO: Handle The collision with the Asteroid
    */
    if (m_BoxCollider)
    {
        ...
        for (auto &item : collidingItems)
        {
            ...
            if (item->GetTag() == "Asteroid")
            {
                std::cout << "Collided with an Asteroids :D\n";
                if (m_Controller)
...
void PlayerController::HandleControls(const sf::Time &elapsed)
{
    if (m_Alive)
    {
        ...
        Vec2f dir = Vec2f((float)cos(toRadian(m_Angle)), (float)sin(toRadian(m_Angle)));
        ...
        float speed = (float)m_Velocity.Length();
        if (speed > m_MaxSpeed)
        ...
        if (finalPosition.x > windowSize.x)
            ...
            finalPosition.x = (float)windowSize.x;

        if (finalPosition.y > windowSize.y)
            finalPosition.y = 0.f;
        else if (finalPosition.y < 0.f)
            finalPosition.y = (float)windowSize.y;
