...
public:
	...
	CustomScene(Engine& engine, b2World& world)
		: Scene(engine, world)
	{
		Main();
	}
	void Main() override
	{
		Entity* newEntity = m_LCManager.CreateEntity("NewEntity");
		Transform& trans = newEntity->GetCopmonent<Transform>();
		trans.size = { 50.0f, 50.0f };
		trans.position = { 1.0f, 1.0f };
		RigidBody2D& rb = newEntity->AddComponent<RigidBody2D>(&this->m_PhysicsWorld);
		BoxCollider& boxCol = newEntity->AddComponent<BoxCollider>(&this->m_PhysicsWorld);
...
public:
	Sandbox()
		: Engine()
	{
		std::unique_ptr<CustomScene> scene = std::make_unique<CustomScene>(*this, *m_PhysicsWorld);
		this->PushStartingScene(std::move(scene));
	}
	~Sandbox()
	{
	}
